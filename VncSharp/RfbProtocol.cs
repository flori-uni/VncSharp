...
namespace VncSharp
{
    ...
    public class RfbProtocol
    {
        ...
        /// <param name="port">
        ///     The Port number on which to connect.  Usually this will be 5900, except in the case that the VNC
        ///     Host is running on a different Display, in which case the Display number should be added to 5900 to determine the
        ///     correct port.
        /// </param>
        public void Connect(string host, int port)
        {
            ...
            tcp = new TcpClient
            {
                NoDelay = true, ReceiveTimeout = RECEIVE_TIMEOUT, SendTimeout = SEND_TIMEOUT
            }; // turn-off Nagle's Algorithm for better interactive performance with host.

            // set receive timeout (15s default)
            // set send timeout to (15s default)
        ...
        /// <summary>
        ///     Send the Protocol Version supported by the client.  Will be highest supported by server (see RFB Doc v. 3.8 section
        ///     6.1.1).
        /// </summary>
        ...
        public byte[] ReadSecurityTypes()
        {
            ...
            else
            {
                ...
                for (var i = 0; i < num; ++i) types[i] = Reader.ReadByte();
            }
        ...
        /// <summary>
        ///     If the server has rejected the connection during Authentication, a reason is given. See RFB Doc v. 3.8 section
        ///     6.1.2.
        /// </summary>
        ...
        {
            Debug.Assert(type >= 1, "Wrong Security Type",
                "The Security Type must be one that requires authentication.");
        ...
        /// <summary>
        ///     Reads the server's Initialization message, specifically the remote Framebuffer's properties. See RFB Doc v. 3.8
        ///     section 6.1.5.
        /// </summary>
        ...
        /// </summary>
        /// <param name="buffer">
        ///     A Framebuffer telling the server how to encode pixel data. Typically this will be the same one
        ...
        ///     An array of integers indicating the encoding types supported.  The order indicates preference,
        ///     where the first item is the first preferred.
        /// </param>
        public void WriteSetEncodings(int[] encodings)
        {
            var buff = new byte[encodings.Length * 4 + 4];
            var x = 0;
        ...
        public void WriteFramebufferUpdateRequest(ushort x, ushort y, ushort width, ushort height, bool incremental)
        {
            ...
            BitConverter.GetBytes(x).Reverse().ToArray().CopyTo(buff, 2);
            BitConverter.GetBytes(y).Reverse().ToArray().CopyTo(buff, 4);
            BitConverter.GetBytes(width).Reverse().ToArray().CopyTo(buff, 6);
            BitConverter.GetBytes(height).Reverse().ToArray().CopyTo(buff, 8);
            writer.Write(buff);
            writer.Flush();
        }
        ...
        {
            var buff = new byte[text.Length + 8];
        ...
        protected sealed class BigEndianBinaryReader : BinaryReader
        {
            ...
            {
                FillBuff(4);
                return ((uint) buff[3] & 0xFF) | ((uint) buff[2] << 8) | ((uint) buff[1] << 16) |
                       ((uint) buff[0] << 24);
            }
        ...
        {
            private readonly BinaryReader uncompressedReader;
            private readonly ZOutputStream zlibDecompressedStream;
            private readonly MemoryStream zlibMemoryStream;
        ...
        public const int TIGHT_ENCODING = 7;
        public const int ZRLE_ENCODING = 16;
        public const int PSEUDO_LAST_RECT_ENCODING = -224;
