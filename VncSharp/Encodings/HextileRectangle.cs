        ...
        public override void Decode()
        {
            ...
            for (var ty = 0; ty < rectangle.Height; ty += 16)
            {
                ...
                for (var tx = 0; tx < rectangle.Width; tx += 16)
                {
                    ...
                    {
                        if ((subencoding & BACKGROUND_SPECIFIED) != 0) backgroundPixelValue = preader.ReadPixel();

                        // Fill-in background colour
                        FillRectangle(new Rectangle(tx, ty, tw, th), backgroundPixelValue);

                        if ((subencoding & FOREGROUND_SPECIFIED) != 0) foregroundPixelValue = preader.ReadPixel();
                        ...
                        if ((subencoding & ANY_SUBRECTS) != 0)
                        {
                            ...
                            for (var i = 0; i < numSubrects; i++)
                            {
                                ...
                                xANDy = rfb
                                    .ReadByte(); // X-position (4 bits) and Y-Postion (4 bits) of this sub rectangle in the tile
                                widthANDheight =
                                    rfb.ReadByte(); // Width (4 bits) and Height (4 bits) of this sub rectangle
